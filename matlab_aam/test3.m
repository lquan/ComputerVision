clear;clc;
files = dir('./data/training/bs00*_N_N_0.png');
% 
% %% loads the images landmarks and saves them in mat format for LoadNiceDataSetContours
% p = struct;
% for i=1:numel(files)
%     file = files(i);
%     p.I = imread(file.name);
%     lmstruct = BO_readlm([file.name(1:end-4) '.lm2']);  %-4 to delete extension
%    
%     p.y = lmstruct.Location(1,:);
%     p.x =  lmstruct.Location(2,:);
%     
%     p.n = length(p.x);
%     p.t = zeros(1,length(p.x));
%     save([file.name(1:end-4) '.mat'], 'p'); 
% end
...
for i=1:2
	...
	TrainingData(i).I=im2double(imread(['./data/training/' filename '.png']));
